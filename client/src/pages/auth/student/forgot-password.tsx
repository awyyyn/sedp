...
export default function StudentForgotPassword() {
  ...
  const [api, setApi] = useState("/reset-password");
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (role !== "STUDENT") {
        setApi("/admin-reset-password");
      } else {
        setApi("/reset-password");
      }
      setStep((stp) => stp + 1);
    } catch (error) {
      toast.error((error as Error).message, {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      setLoading(true);
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/auth/${api}`,
        {
          method: "POST",
      ...
      localStorage.removeItem("passwordAccessToken");
      toast.success("Password Reset Successful", {
        richColors: true,
        dismissible: true,
        duration: 5000,
        position: "top-center",
        icon: <Icon icon="solar:verified-check-bold" />,
      });
      setStep((stp) => stp + 1);
    } catch (error) {
      toast.error((error as Error).message, {
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>Forgot Password | SEDP</title>
        <meta
          name="description"
