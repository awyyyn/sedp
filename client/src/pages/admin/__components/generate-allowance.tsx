...
const GenerateAllowanceSchema = yup.object({
  ...
  miscellaneousAllowance: yup
    ...
    .max(2000, "Miscellaneous allowance exceeded the maximum of 2000"),
  thesisAllowance: yup
    .number()
    .typeError("Invalid Amount")
    .optional()
  ...
  const formik = useFormik({
    ...
    onSubmit: async (_, helpers) => {
      const miscFee =
        ...
        getDocsTotalAmount(formik.values.docs || [], "MISCELLANEOUS_ALLOWANCE");

      if (miscFee > 2000) {
        helpers.setFieldError(
          "miscellaneousAllowance",
          `Total Amount: ${formatCurrency(miscFee)} exceeded the maximum of ${formatCurrency(2000)}`,
        );
